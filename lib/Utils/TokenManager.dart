...
class TokenManager {
  ...
  static Future<void> storeTokens({
    ...
    try {
      ...
      if (refreshToken != null && refreshToken.isNotEmpty) {
        ...
        // Log if refresh token is missing - this might be why auto-logout happens
  ...
  static Future<bool> refreshAccessToken() async {
    ...
    try {
      ...
      // Clear the future when done so future refreshes can start
      _refreshFuture = null;
    }
  }
  ...
  static Future<String?> getValidAccessToken() async {
    try {
      ...
      if (await isTokenExpiredOrExpiringSoon()) {
        ...
        if (!refreshSuccess) {
          log('❌ Could not get valid token: Refresh failed');
          // Note: We don't call clearTokens here to allow ApiService to handle 401
          // if the server happens to still accept the current token.
          return null;
        }
      }
