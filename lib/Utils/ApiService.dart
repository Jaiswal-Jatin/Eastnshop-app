...
import 'package:eastnshop/Utils/AuthMiddleware.dart';
...
class ApiService {
  ...
  static Future<bool> handleResponse(http.Response response) async {
    if (response.statusCode == 401) {
      ...
      if (!refreshSuccess) {
        ...
        await TokenManager.clearTokens();
        await AuthMiddleware.handleAuthError();
        return false;
      }

      // Token refreshed successfully, but the current request failed
  ...
  static Future<Map<String, dynamic>> subscribeToPlan(String plan) async {
    try {
      // Get current user info
      Map<String, dynamic>? userInfo = await getCurrentUser();
      ...
      Map<String, dynamic> requestBody = {'userId': userId, 'plan': plan};
