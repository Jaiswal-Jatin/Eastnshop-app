...
import 'package:eastnshop/Controllers/InactiveOffersController.dart';
...
class _InactiveOffersPageState extends State<InactiveOffersPage> {
  final InactiveOffersController controller = Get.put(InactiveOffersController());
  ...
  void _showReactivateConfirmation(BuildContext context, Map<String, dynamic> offer) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Are you sure you want to reactivate "${offer['product_name']?.toString() ?? 'this offer'}"?'),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('Are you sure you want to permanently delete "${offer['product_name']?.toString() ?? 'this offer'}"? This action cannot be undone.'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: const Text('Cancel'),
                              ...
                              children: [
                               ...
                               Spacer(),
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: getActionStatusColor(offer['action']?.toString()),
                                    borderRadius: BorderRadius.circular(4),
                                  ),
                                  child: Text(
                                    getActionStatusText(offer['action']?.toString()),
                                    style: const TextStyle(
                                      fontSize: 9,
                                      fontWeight: FontWeight.bold,
                                      color: Colors.white,
                                ...
                                /// Action Buttons - Only show reactivate if action is "accepted"
                                if (offer['action']?.toString().toLowerCase() == 'accepted')
                                  Row(
                                    children: [
                                      Expanded(
                                        child: Container(
                                          ...
                                          child: IconButton(
                                            ...
                                            onPressed: () => onReactivateOffer(context, offer),
                                            tooltip: "Reactivate",
                                          ),
                                        ),
                                      ),
