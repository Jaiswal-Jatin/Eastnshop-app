...
class OfferController extends GetxController {
  ...
  Future<void> fetchShops() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        log("❌ Failed to fetch shops: ${response.statusCode} - ${response.body}");
        // AppSnackBar.show(
  ...
  Future<void> fetchOfferTypes() async {
    try {
      ...
      final response = await ApiService.get('/api/offer-types');
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          offerTypes.value = List<Map<String, dynamic>>.from(data['offer_types']);
          log("✅ Fetched ${offerTypes.length} offer types from 'offer_types' field");
          for (int i = 0; i < offerTypes.length; i++) {
            log("Offer Type $i: ${offerTypes[i]['type_name']}");
          }
        } else {
        ...
        log("❌ Failed to fetch offer types: ${response.statusCode} - ${response.body}");
