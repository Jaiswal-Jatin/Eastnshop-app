...
class OfferDetailsController extends GetxController {
  ...
  Future<void> _fetchOfferDetailsWithRetry(String offerId, {int retryCount = 0}) async {
    try {
      ...
      final response = await ApiService.get('/api/offer/$offerId');
  ...
  // Format time from HH:mm:ss or HH:mm AM/PM to HH:mm AM/PM
  String _formatTime(String time) {
    if (time.isEmpty) return '';
    
    // If it already contains AM or PM, just return it
    ...
    try {
      ...
      log("Error formatting time $time: $e");
    }
    return time;
  }
  ...
  bool get isShopOpen {
    ...
    for (var hour in hours) {
      final day = hour['day']?.toString() ?? '';
      String open = hour['open']?.toString() ?? '';
      String close = hour['close']?.toString() ?? '';
      
      if (day == currentDay && open.isNotEmpty && close.isNotEmpty) {
        // Ensure comparison is done in 24-hour format
        String open24 = _to24h(open);
        String close24 = _to24h(close);
        
        if (currentTime.compareTo(open24) >= 0 && currentTime.compareTo(close24) <= 0) {
          return true;
        }
      }
    }
  ...
  // Helper to ensure a time string is in HH:mm:ss format for comparison
  String _to24h(String time) {
    if (time.isEmpty) return "00:00:00";
    
    // If it's already HH:mm:ss, return as is
    if (!time.toUpperCase().contains('AM') && !time.toUpperCase().contains('PM')) {
      // Ensure it has seconds if missing
    ...
    try {
      // Parse 12h format
      final format = DateFormat.jm(); // matches "9:00 AM" or "09:00 AM"
      final date = format.parse(time.trim());
      return DateFormat('HH:mm:ss').format(date);
    } catch (e) {
      log("Error converting to 24h: $time - $e");
