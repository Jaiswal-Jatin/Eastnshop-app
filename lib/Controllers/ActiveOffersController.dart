...
class ActiveOffersController extends GetxController {
  ...
  void filterOffersByCategory() {
    ...
    log("✅ Categorized offers into ${sortedOffers.length} categories: ${sortedOffers.keys.toList()}");
  }
  ...
  Future<void> refreshAllData() async {
    ...
    if (selectedShopId.value.isNotEmpty) {
      log("🔄 Shop selected, fetching offers for shop: ${selectedShopId.value}");
      await fetchOffersByShopId(selectedShopId.value);
    } else {
  ...
  // Edit offer
  Future<bool> editOffer(Map<String, dynamic> offerData, {List<File>? imageFiles}) async {
    try {
      isLoadingOffers.value = true;
      errorMessage.value = '';
  ...
  Future<bool> deleteOffer(String offerId) async {
    try {
      ...
      var response = await ApiService.patch('/api/offer/inactive/$offerId', body: {});
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        ...
        try {
          dynamic errorData = jsonDecode(response.body);
          String errorMsg = errorData['message'] ?? "Failed to make offer inactive";
          errorMessage.value = errorMsg;
          log("❌ API Error: $errorMsg");
          // AppSnackBar.show(
          //   message: errorMsg,
  ...
  Future<void> testDeleteEndpoints(String offerId) async {
    ...
    for (String endpoint in endpoints) {
      try {
        ...
        log("✅ $endpoint - Status: ${response.statusCode}, Body: ${response.body}");
      } catch (e) {
